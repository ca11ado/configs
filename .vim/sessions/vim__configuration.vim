...
nnoremap ,ss :mks! ~/.vim/sessions/*.vim<BS><BS><BS><BS><BS>
map ,l :!clear; and eslint %
map ,z 
map ,b :!browser-sync start --no-notify --no-ui --server --files % > /dev/null 2>&1 &
map ,c :%w !pbcopy
map ,et :tabe =expand("%:p:h") . "/" 
map ,ev :vsp =expand("%:p:h") . "/" 
map ,es :sp =expand("%:p:h") . "/" 
map ,ew :e =expand("%:p:h") . "/" 
...
silent only
silent tabonly
cd ~/Projects/github-repositories/javascript-tutoriall/ru.javascript.info/1-js/06-advanced-functions/03-closure
...
argglobal
%argdel
$argadd ~/.vim/modules/vim-common.vim
edit ~/.vim/modules/vim-common.vim
...
wincmd w
argglobal
if bufexists("~/.vim/plugins.vim") | buffer ~/.vim/plugins.vim | else | edit ~/.vim/plugins.vim | endif
let s:cpo_save=&cpo
set cpo&vim
imap <buffer> <S-BS> <Plug>delimitMateS-BS
vnoremap <buffer> <silent> [" :exe "normal! gv"|call search('\%(^\s*".*\n\)\%(^\s*"\)\@!', "bW")
...
1
normal! 0
lcd ~/Projects/github-repositories/javascript-tutoriall/ru.javascript.info/1-js/06-advanced-functions/03-closure
wincmd w
2wincmd w
exe 'vert 1resize ' . ((&columns * 101 + 102) / 204)
exe 'vert 2resize ' . ((&columns * 102 + 102) / 204)
tabnext 1
badd +0 ~/.vim/modules/vim-common.vim
badd +1 ~/.vim/modules
badd +0 ~/.vim/plugins.vim
