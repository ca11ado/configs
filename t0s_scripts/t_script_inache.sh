...
PROJECT_DIR=/var/www/nodeStatic/inache
HOME_DIR=/home/t0s
LOG_DIR=$HOME_DIR/node-server-log
NODE_SERVER_NAME_INACHE="serverNode.js"
NODE_SERVER_NAME_STUB="serverNode_stub-page.js"
...
if [ $# -gt 0 ]; then
  ...
  if [ "$1" == "inache" ]; then
    if [ "$2" == "-start" ]; then
      run_node_server ${PROJECT_DIR} ${NODE_SERVER_NAME_INACHE}
    fi
    if [ "$2" == "-stop" ]; then
      get_server_pid ${PROJECT_DIR}/${NODE_SERVER_NAME_INACHE} | xargs kill
    fi
    if [ "$2" == "-restart" ]; then
      get_server_pid ${PROJECT_DIR}/${NODE_SERVER_NAME_INACHE} | xargs kill
      run_node_server ${PROJECT_DIR} ${NODE_SERVER_NAME_INACHE}
  ...
  if [ "$1" == "stub" ]; then
    if [ "$2" == "-start" ]; then
      run_node_server ${PROJECT_DIR} ${NODE_SERVER_NAME_STUB}
    fi
    if [ "$2" == "-stop" ]; then
      get_server_pid ${PROJECT_DIR}/${NODE_SERVER_NAME_STUB} | xargs kill
    fi
    if [ "$2" == "-restart" ]; then
      get_server_pid ${PROJECT_DIR}/${NODE_SERVER_NAME_STUB} | xargs kill
      run_node_server ${PROJECT_DIR} ${NODE_SERVER_NAME_STUB}
    ...
    if [ "$2" == "-status" ]; then
      re='^[0-9]+$'
      SERVER_PID=$(get_server_pid ${PROJECT_DIR}/${NODE_SERVER_NAME_STUB})
      if [[ $SERVER_PID =~ [[:digit:]] ]]; then
        echo "server pid: ${SERVER_PID}"
      else
        echo "node server is not running: ${SERVER_PID}"
